...
function removeFirstOccurrences(/* str, value */) {
  ...
  // const char = str.search(value);
  // return str.slice(0, char).concat(str.slice(char + value.length, str.length));
}
...
function startsWith(str, substr) {
  // throw new Error('Not implemented');
  // return str.startsWith(substr) ? true : false;
...
function formatTime(minutes, seconds) {
  ...
  let endMin;
  let endSec;
  // min.length === 1 ? endMin = min.padStart(2,'0') : endMin = min;
  // sec.length==1 ? endSec = sec.padStart(2,'0'): endSec = sec;
  // return endMin+':'+endSec
  if (min.length === 1) {
    endMin = min.padStart(2, '0');
  } else {
    endMin = min;
  }
  if (sec.length === 1) {
    endSec = sec.padStart(2, '0');
  } else {
    endSec = sec;
  }
  return endMin.concat(':').concat(endSec);
}
...
function reverseString(str) {
  ...
  return String(arr.reverse()).split(',').join('');
}
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  const newStr = str.split(',').join('').toLowerCase().replaceAll(' ', '');
  let clearStr;
  if (newStr.includes('!')) {
    clearStr = newStr.replace('!', '');
...
function reverseWords(str) {
  // throw new Error('Not implemented');
  const arr = str.split(' ');
  return String(arr.map((el) => el.split('').reverse().join(''))).replaceAll(
    ',',
    ' '
  );
}
...
function encodeToRot13(/* str */) {
  ...
  // const arrVal = Array.from(str).map((el) => {
  //   for (let i = 0; i < alphabetUp.length; i += 1) {
  //     if (el === alphabetDown[i]) {
  //       el = alphabetDown[i + 13];
  //       return el;
  //     } else if (el === alphabetUp[i]) {
  //       el = alphabetUp[i + 13];
  //       return el;
  //     }
  //   }
  // закольцевать
  // });
  // return String(arrVal.slice(0, arrVal.length - 1))
  //   .split(',')
  //   .join('');
}
